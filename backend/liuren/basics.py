...
# 天德（月德之神，基于月支）— 以地支显示
# 寅(正):未  卯(二):申  辰(三):亥  巳(四):戌  午(五):亥  未(六):寅
# 申(七):丑  酉(八):寅  戌(九):巳  亥(十):辰  子(十一):巳  丑(十二):申
TIANDE_MAP = {
    "寅": "未", "卯": "申", "辰": "亥", "巳": "戌",
    "午": "亥", "未": "寅", "申": "丑", "酉": "寅",
    "戌": "巳", "亥": "辰", "子": "巳", "丑": "申",
...
# 月德（三合局帝旺之干所寄之支）
# 寅午戌(火)→丙寄巳  亥卯未(木)→甲寄寅  申子辰(水)→壬寄亥  巳酉丑(金)→庚寄申
YUEDE_MAP = {
    ("寅", "午", "戌"): "巳",
    ("亥", "卯", "未"): "寅",
    ("申", "子", "辰"): "亥",
...
def get_yuede(yue_zhi: str) -> str:
    for key, val in YUEDE_MAP.items():
        if yue_zhi in key:
            return val
    return ""
