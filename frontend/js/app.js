...
function loadCaseList() {
    ...
    cases.forEach(function(c) {
        ...
        div.innerHTML = '<input type="checkbox" class="case-cb" value="'+c.id+'">' +
            ...
            '<button class="btn btn-sm" onclick="var w=window.open(\'\',\'_blank\');if(w)w.location.href=\'/api/cases/'+c.id+'/export-html\';else window.location.href=\'/api/cases/'+c.id+'/export-html\'" style="background:rgba(212,160,23,0.06);border:1px solid rgba(212,160,23,0.25);color:#8b6914">导出</button>';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function exportCaseHTML(caseId) {
        ...
        if (!caseObj || !caseObj.pan_data) {
            try {
                var resp = await fetch('/api/cases/' + caseId);
                var j = await resp.json();
                if (!j.success || !j.case) { alert('案例数据缺失，无法导出'); return; }
                caseObj = j.case;
                if (!caseObj.personal_notes && caseObj.notes) caseObj.personal_notes = caseObj.notes;
            } catch(e) { alert('获取案例数据失败：' + e.message); return; }
        }
        if (!caseObj.pan_data) { alert('案例数据缺失（无盘面）'); return; }
